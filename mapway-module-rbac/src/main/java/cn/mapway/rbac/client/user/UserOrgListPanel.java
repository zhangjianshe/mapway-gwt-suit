 ...
 * 用户的所有所属机构
 */
public class UserOrgListPanel extends ZTree {
    private ClickHandler favoriteHandler=new ClickHandler() {
        @Override
        public void onClick(ClickEvent event) {
            IData data = (IData) event.getSource();
            RbacUserOrg org = (RbacUserOrg) data.getData();
            if(org.major!=null || org.major){
                org.major=false;
            }
        }
    };
    ...
    private void renderItems(List<RbacUserOrg> userOrgs) {
        ...
        for (RbacUserOrg org : userOrgs) {
            ...
            if (org.major != null && org.major) {
                icon.setIconUnicode(Fonts.FAVORITE_ON);
            } else {
                icon.setIconUnicode(Fonts.FAVORITE_OFF);
            }
            item.appendWidget(icon, 28);
            icon.addClickHandler(favoriteHandler);
            icon.setData(org);
